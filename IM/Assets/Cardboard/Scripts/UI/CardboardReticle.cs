...
public class CardboardReticle : MonoBehaviour, ICardboardPointer
{
    ...
    private float reticleDistanceInMeters = 5.0f;
    ...
    private const float kReticleDistanceMax = 5.0f;
    ...
    public void OnGazeStart(Camera camera, GameObject targetObject, Vector3 intersectionPosition)
    {
        ...
        if (inRange)
        {
            if (targetObject.gameObject.CompareTag("Star"))
            {
                targetObject.gameObject.SetActive(false);
                count++;
                updateScore();
            }
            else if (targetObject.gameObject.CompareTag("Diamond"))
            {
                targetObject.gameObject.SetActive(false);
                count += 5;
                updateScore();
            }
            else if (targetObject.gameObject.CompareTag("Box"))
            {
                targetObject.gameObject.SetActive(false);
